...
model0 = ar_table.loc['const', 'coef'] + ar_table.loc['phi', 'coef'] * np.linspace(-0.025, 0.150)
...
ar_1 = ax0.plot(np.linspace(-0.025, 0.150), model0, color="red", lw=1.5, label='Persistence')
...
reg = ar_j(vintage_trim)

# Generate AR(1) predictions
ar_predictions = reg.predict(start=vintage_trim.index[0], end=vintage_trim.index[-1])
