...
axi.plot(np.linspace(-0.1, 0.1), model_p, label='POOLED', color='red', lw=1.5)
...
vintage_trim = vintage_trim[['t0', 't1', 't2', 't3', 't4']]

model0 = ar_table.loc[0, 'coef'] * np.linspace(-0.025, 0.150)

fig, ((ax0, ax1), (ax2, ax3)) = plt.subplots(nrows=2, ncols=2, figsize=(10, 6), sharey=True,)
fig.suptitle('Persistence', fontsize=14)

ax0.scatter(vintage_trim['t0']['1965-09-30':], vintage_trim['t0'].shift(1)['1965-09-30':], c='blue', label='t = 1', alpha=1, s=20)
ar_1 = ax0.plot(np.linspace(-0.025, 0.150), model0, color="red", lw=1.5, label='AR(1)')
ax0.set_ylabel('Infl. (t-1)', fontsize=12)
ax0.legend(['t = 1'])
...
ax2.set_ylabel('Infl. (t-1)', fontsize=12)
ax2.set_xlabel('Infl. (t)', fontsize=12)
ax2.legend(['t = 3'])
...
ax3.set_xlabel('Infl. (t)', fontsize=12)
ax3.legend(['t = 4'])

fig.legend(handles=ar_1)
plt.tight_layout()
plt.show()
fig.savefig('output/ar_scat.png')
